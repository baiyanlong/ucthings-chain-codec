...
import ucthings.codec.common.util.ReflectUtil;
 ...
 * @author L.cm
 */
public class BeanMapEmitter extends ClassEmitter {
	private static final Type BEAN_MAP = TypeUtils.parseType(SimpleBeanMap.class.getName());
	...
	public BeanMapEmitter(ClassVisitor v, String className, Class type, int require) {
		...
		if (require != 0) {
			for (Iterator it = allProps.keySet().iterator(); it.hasNext(); ) {
				...
				if ((((require & SimpleBeanMap.REQUIRE_GETTER) != 0) && !getters.containsKey(name)) ||
						(((require & SimpleBeanMap.REQUIRE_SETTER) != 0) && !setters.containsKey(name))) {
					it.remove();
					getters.remove(name);
					setters.remove(name);
				}
